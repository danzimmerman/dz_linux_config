...
if [[ ! -f ~/.bashrc.original && -f ~/dz_linux_config/bash_config/.bashrc_additions ]]; then
    echo "Making a read-only copy of ~/.bashrc to ~/.bashrc.original"
    cp ~/.bashrc ~/.bashrc.original
    sudo chmod a-w ~/.bashrc.original
    cat ~/dz_linux_config/bash_config/.bashrc_mod_tail >> ~/.bashrc
else
    echo "~/.bashrc.original backup copy already exists. Remove it if this is not expected."
fi

if [[ -d ~/dz_linux_config && ! -f ~/.inputrc && ! -f ~/.bash_aliases ]]; then
  echo "~/dz_linux_config exists but ~/.inputrc and ~/.bash_aliases do not, making softlinks..."
  ln -s ~/dz_linux_config/bash_config/.bash_aliases ~/.bash_aliases
  ln -s ~/dz_linux_config/bash_config/.inputrc ~/.inputrc
else
...
#sudo cp ~/dz_linux_config/network/99-eth0-dhcp.yaml /etc/netplan
